...
extern int LoadFileMem( const char *Path, void **DataBuf, size_t *Size ) ;		// ファイルの内容をメモリに読み込む( 0:成功  -1:失敗 )
extern int LoadFileMem( const char *Path, void *DataBuf,  size_t *Size ) ;		// ファイルの内容をメモリに読み込む( 0:成功  -1:失敗 )
extern int SaveFileMem( const char *Path, void *Data,     size_t  Size ) ;		// メモリの内容をファイルに書き出す 
...
extern int SetExName( const char *Path, const char *ExName, char *DestBuf ) ;	// 拡張子を変更する
